...
def load_urls():
    ...
    try:
        ...
        with open("bot/url/anime.txt", "r", encoding="utf-8-sig") as f:
...
def register_anime(bot: commands.Bot):
    ...
    async def anime(interaction: discord.Interaction):
        ...
        try:
            ...
            if not video_urls:
                if error_channel:
                    await error_channel.send("❌ Danh sách video anime trống!")
            ...
            for attempt in range(min(MAX_RETRIES, len(video_urls))):
                ...
                try:
                    ...
                    await interaction.followup.send(
                        f"🎬 Video anime cho {interaction.user.mention} ({actual_size_mb:.2f}MB):", 
                        file=video_file
                    )
                    return  # Success!
            ...
            # Thất bại hoàn toàn
            await interaction.followup.send(
                f"❌ Không thể tải video anime. Đã thử {len(used_urls)} video nhưng đều lỗi!"
            )
